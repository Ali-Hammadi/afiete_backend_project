...
def approve_certificates(modeladmin, request, queryset):
    for obj in queryset:
        obj.status = 'approved'
        obj.save()
    Thread(target=send_professional_email, args=(obj.doctor.user.email, "Doctor Accepted")).start()
...
def reject_certificates(modeladmin, request, queryset):
    for obj in queryset:
        obj.status = 'rejected'
        obj.save()
    Thread(target=send_professional_email, args=(obj.doctor.user.email, "Doctor Rejected")).start()
